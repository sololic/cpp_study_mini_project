...
```cpp
auto x = 10;  // int로 추론, 고정
x = 20;       // OK — 값 변경은 자유롭게 가능
x = 3.14;     // 에러! 타입 변경은 불가
```

### auto로 선언 후 연산

추론된 타입 그대로 산술 변환 규칙이 적용된다.

```cpp
auto a = 10;          // int
auto b = 3.14;        // double
auto result = a + b;  // int + double → double (더 큰 타입으로 변환)
```

### 단점

```cpp
auto result = someFunction();  // 타입이 뭔지 바로 안 보임 → 가독성 저하
auto x = 3.14;                 // float로 쓰고 싶었는데 double로 추론될 수 있음
```

타입이 명확할 땐 명시적으로 쓰고, 복잡한 타입(`iterator` 등)에서만 쓰는 게 권장된다.
...
## 📖 const & constexpr — 상수 선언

### const — 런타임 상수

한 번 값이 정해지면 변경 불가. 값이 **언제** 정해지는지는 상관없다.

```cpp
const int MAX_SIZE = 100;       // OK — 컴파일 시점에 값 확정
const int age = getAge();       // OK — 실행 중에 값이 정해져도 됨
age = 30;                       // 에러! 한번 정해지면 변경 불가
```

### constexpr — 컴파일 타임 상수

**컴파일할 때 이미 값이 확정**되어야 한다. `const`보다 더 엄격.

```cpp
constexpr int MAX = 100;        // OK — 컴파일 시점에 값 확정
constexpr int age = getAge();   // 에러! 실행 중에 정해지는 값은 불가
```
...
| | `const` | `constexpr` |
|---|---|---|
| 값 확정 시점 | 런타임도 OK | 반드시 컴파일 타임 |
| 변경 가능 여부 | 불가 | 불가 |
| 최적화 | 보통 | 컴파일러가 더 적극적으로 최적화 |

### Python과 차이

```python
MAX = 100   # 관례상 대문자로 쓰지만 변경 가능
MAX = 200   # 아무 문제 없음
```

C++의 `const`는 컴파일러가 변경 자체를 막아준다.
